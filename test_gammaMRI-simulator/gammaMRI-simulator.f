...
! detectors.
!
! gammaMRI_test.f: We prepare the code to read simple input files
...
!      dt=poisson_time(act0)
      ...
      print*,'Gyromagnetic ratio [gyro] (Hz/T)=',gyro
      print*,'Static magnetic field [B0] (T)=',B0
      print*,'Larmor frequency [gyro*B0] (Hz)=',wlarmor
      !print*,'Larmor frequency (rad/s)=',wl_rad
...
!---------------------------------------------------------------------
     ...
     o phizdet_dist(nphibins,nzbins)
      ...
      print*,"------------------------------------"
      print*,"This code generates the output data from a distribution ",
     o "of polarized nucleii."
      print*,"Let's read input file: input_gammaMRI.inp"
      open(13,file='input_gammaMRI.inp')
      read(13,*)nx
      write(*,*)'nx=',nx
      read(13,*)ny
      write(*,*)'ny=',ny
      read(13,*)xfov
      write(*,*)'X-FOV size (cm)=',xfov
      read(13,*)yfov
      write(*,*)'Y-FOV size (cm)=',yfov
      read(13,*)det_radius
      write(*,*)'Radius of detectors ring (cm)=',det_radius
      read(13,*)det_height
      write(*,*)'Detector size (cm)=',det_height
      read(13,*)ndetectors
      write(*,*)'Total number of detectors: ',ndetectors
      read(13,*)act
      write(*,*)'Initial activity (Bq)=',act
      read(13,*)amlife
      write(*,*)'Isotope half life (s)=',amlife
      read(13,*)a2
      write(*,*)'Polarization (a2)=',a2
      read(13,*)file_magvectors
      write(*,*)'Magnetization vectors file:',trim(file_magvectors)
      read(13,*)time_step
      write(*,*)'Time step (in magnetization file) (s)=',time_step
      read(13,*)nt
      write(*,*)'Number of time bins (in magnetization file) =',nt
      read(13,*)act_file
      write(*,*)'File with activity distribution: ',trim(act_file)
      close(13)
...
!------
      ...
      open(15,file='list_prob-map_gammaMRI.raw',form='unformatted',
     o access='stream')
      open(16,file='list_det_gammaMRI.txt')
      time=0.
      ndecays=0
      ndetected=0
      prob_maptot=0.
     ...
     o   xdet,ydet,zdet,idet,prob_map)
       ...
       if(akk.ge.0)then !detection
        ...
        write(16,"(I10,2F17.7,2I10,6F17.7,I10)")ndecays,time,act,ix,iy,
     o   xem,yem,xdet,ydet,zdet,phi,idet
...
!     o    prob_map_det(ix,iy,idet)+1
        prob_map_det(ix,iy,idet)=prob_map_det(ix,iy,idet)+1
       endif
      enddo
      print*,"Total number of simulated decays = ",ndecays
      print*,"Total number of detected events = ",ndetected
      print*,"Sensitivity = ",real(ndetected)/real(ndecays)
      print*,"End time (s) = ",time
      print*,"End activity (Bq) = ",act
      !close(13)
      !close(14)
      close(15)
      close(16)

      open(13,file='events_per_detector.txt')
      write(13,*)"#Detector  Angular_pos(rad)  Events  Sensitivity"
      do i=1,ndetectors
     ...
     o  real(ndetections_det(i))/ndecays
      ...
      open(13,file='activity_distribution.txt')
      write(13,*)"#Time_bin  Time(s)  Decays  Activity(Bq)"
      do i=1,ntbins
...
!      write(13,*)"#Zbin   Z_position(cm)  Events   Events/cm"
!      do i=1,nzbins
...
!      close(13)
      ...
      open(13,file='Total_prob-map_gammaMRI.raw',access='stream',
     o form='unformatted')
      prob_map=prob_maptot
      write(13)prob_map !tot
      close(13)

      open(13,file='emissions_image.raw',access='stream',
     o form='unformatted')
      write(13)em_image
      close(13)

      open(13,file='detections_image.raw',access='stream',
     o form='unformatted')
      write(13)det_image
      close(13)

      !if(if_reco.ne.1)then
      open(13,file='detections_per_detector_image.raw',access='stream',
     o form='unformatted')
